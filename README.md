...
- **ホーム**：登録した日付からの経過（◯年◯ヶ月◯日）を `group` ごとに表示
  - 人生の節目（`milestone`）：記念日・節目の日（100日、1,000日…）もお知らせ
  - 仕事・ビジネス（`business`）：業務委託や事業の開始日。人生の節目と同じく周年・節目の日をお知らせ
  - 買ったもの（`purchase`）／契約・入会（`contract`）／定期的なこと（`routine`）／その他（`other`）
...
誕生日（当日と事前リマインド）と「人生の節目」の記念日・節目の日を、毎朝 8:00（JST）ごろ Web Push で 1 通にまとめて送ります（該当がない日は送りません）。
